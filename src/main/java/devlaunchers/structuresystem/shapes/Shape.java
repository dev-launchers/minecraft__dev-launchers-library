...
public abstract class Shape {
    ...
    public BlockVector transformVector(BlockVector vec) {
        ...
        return new BlockVector(
                Math.round(vec.getX()), Math.round(vec.getY()), Math.round(vec.getZ()));
    }
    ...
    /**
     * Adds a Filter Transformation, which allows cutting off Shapes (for example only lower half of
     * a Sphere etc.)
     *
     * @param filter
